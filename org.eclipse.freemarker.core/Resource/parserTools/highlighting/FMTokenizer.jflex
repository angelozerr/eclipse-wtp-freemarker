...
<ST_BLOCK_TAG_INTERNAL_SCAN> {fmCommentStart}  {
	yybegin(ST_FM_COMMENT);
	assembleEmbeddedContainer(FM_COMMENT_OPEN, FM_COMMENT_CLOSE);
	if(yystate() == ST_BLOCK_TAG_INTERNAL_SCAN)
		yybegin(ST_BLOCK_TAG_SCAN);
...
<ST_FM_COMMENT_END> {fmCommentEnd} {
	if(Debug.debugTokenizer)
...
/* specially treated FM "assign" tag names */
<ST_JSP_CONTENT> assign {
	if(Debug.debugTokenizer)
		dump("#assign FM tag name");//$NON-NLS-1$
	//fEmbeddedHint = XML_TAG_ATTRIBUTE_NAME;
	//fEmbeddedPostState = ST_XML_EQUALS;
      //  yybegin(ST_XML_ATTRIBUTE_NAME);
        return JSP_ROOT_TAG_NAME;
